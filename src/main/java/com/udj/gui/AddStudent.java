...
import com.udj.gui.components.AddSaveConfirm;
...
public class AddStudent extends javax.swing.JDialog {
   ...
   public AddStudent(java.awt.Frame parent, boolean modal, StudentManagement studentPanel) {
        super(parent, modal);
    ...
    private void SaveCMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_SaveCMouseClicked
        ...
        if (id.isEmpty()) {
            id = StudentManager.generateStudentID();
        ...
        if (id.isEmpty()) {
            javax.swing.JOptionPane.showMessageDialog(this,"Error generating student ID.","ID Error", javax.swing.JOptionPane.ERROR_MESSAGE);
            return;
        }
        if (StudentManager.studentIDExists(id)) {
        ...
        AddSaveConfirm dialog = new AddSaveConfirm((java.awt.Frame)null, true);
        dialog.setVisible(true);
        if (!dialog.isConfirmed()) return;

        boolean success = StudentManager.saveStudent(id, name, program, department,
